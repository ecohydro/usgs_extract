...
detection_transform = transforms.Compose([
    ToGrayscale(),
    # ApplyThresholding(),
    ApplyDilation(kernel_size=2),  # Adjust kernel_size as needed
    GrayscaleTo3Channel(),
    MaxResize(800),
    transforms.ToTensor(),
...
def runModel(files, table_path, name_csv):
    file_path = []
    score_list = []
    for path in files:
    ...
    data.to_csv(os.path.join(table_path, name_csv), index=False)
    return data
...
if __name__ == "__main__":
    ...
    try:
        ...
        runModel(files, table_path, "validation_results.csv")
